<template>
  <b-card class="mb-3">
    ...
    <b-list-group>
      <span
        v-if="
          todoList.length === 0 && isDragging === false && loading === false
        "
        class="no-list-item"
      >
      ...
      >
        ...
        >
          <div
            class="todo-name"
            :data-name="`todo-${todo.state}-name-${index}`"
          >
            <span
              class="ml-2"
              :class="{
          ...
          <b-button size="sm" variant="link" @click="removeTodo(todo)">
...
export default {
  ...
  props: {
    loading: Boolean,
    cardHeader: { type: String, default: null },
